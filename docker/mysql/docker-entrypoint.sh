...
if [ "$1" = 'mysqld' ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		"${mysql[@]}" <<-EOSQL
			GRANT ALL PRIVILEGES ON *.* TO 'root'@'node1' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD' WITH GRANT OPTION ;
			GRANT ALL PRIVILEGES ON *.* TO 'root'@'node2' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD' WITH GRANT OPTION ;
			GRANT ALL PRIVILEGES ON *.* TO 'root'@'172.%.0.%' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD' WITH GRANT OPTION ;
			FLUSH PRIVILEGES ;
		EOSQL
